...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/memory-metadata-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[memory-metadata-mcp] Security Report`.
...
- Path traversal in memory file queries that exposes files outside the configured memory directory
- Information disclosure through metadata queries returning sensitive session content
- Dependency vulnerabilities with a plausible exploitation path in memory-metadata-mcp's usage
